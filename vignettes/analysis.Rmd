...
for(mode in c("TL-source","TL-target","MTL-size","TL-prop","MTL-prop")){
  ...
  if(mode=="TL-source"){
    grid[[mode]] <- expand.grid(prob.common=0.05,prob.separate=0.025,family="gaussian",n_source=cand,n_target=100)
  } else if(mode=="TL-target"){
    grid[[mode]] <- expand.grid(prob.common=0.05,prob.separate=0.025,family="gaussian",n_source=100,n_target=cand) 
    ...
    grid[[mode]] <- expand.grid(prob.common=cand,prob.separate=NA,family="gaussian",n_source=100,n_target=100,n0=100)
  } else {
    stop("Wrong mode.")
  }
...
for(i in seq_len(iter)){
  ...
  for(j in seq_len(q)){
    Y[,j] <- abs(y - stats::rbinom(n=n,size=1,prob=0.1))
  }
  auc_mtl[[i]] <- cv.multiple(y=Y,X=x,method=c("glm.separate","sparselink"),family="binomial",alpha.init=0.95,alpha=1,type="exp")$auc
}
...
Y <- X <- list()
for(i in seq_len(iter)){
  cond[y==0] <- sample(rep(x=c(0,1),length.out=sum(y==0)))
  cond[y==1] <- sample(rep(x=c(0,1),length.out=sum(y==1)))
