...
for(mode in c("transfer","multiple")){
  ...
  for(i in seq(from=1,to=nrow(grid))){
    ...
    if(mode=="transfer"){
      ...
      method <- c("wrap_separate","wrap_mgaussian","sparselink","wrap_spls") # add glmnet "mgaussian" (only for linear case) or "MTPS" (not sparse)?
    }
...
for(i in names(project)){
  ...
  for(j in seq_len(5)){ # 5 repetitions of 10-fold CV
    set.seed(j)
    codes <- project[[i]]
    result[[i]][[j]] <- cv_transfer(y=y[codes],X=x[codes],family="binomial",method=c("wrap_separate","wrap_glmtrans","sparselink","wrap_xrnet"))
  }
}
