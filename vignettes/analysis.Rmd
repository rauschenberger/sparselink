...
**We obtained our results on 2024-12-29 using R version 4.4.0 (2024-04-24, "Puppy Cup") running on a Dell Latitude 5440 laptop with a 13th Gen Intel(R) Core(TM) i7-1365U processor (1800 Mhz) and the operating system Microsoft Windows 10 Enterprise. The total computation time was around 2 hours (excluding data download, without parallel computing).**

## Working environment

This chunk verifies the working environment. The working directory, specified by the object `path`, must contain the R functions in "package/R/functions.R" as well as the folders "results" and "manuscript". Alternatively, the R functions can be loaded from the R package `sparselink`. This chunk also installs missing R packages from CRAN or Bioconductor. Running this vignette with the option `eval=FALSE` means that figures and tables are generated from previously obtained results.  

```{r setup,eval=TRUE}
knitr::opts_chunk$set(eval=FALSE) # Use eval=TRUE for reproducing everything.
...
for(mode in c("transfer","multiple")){
  ...
  for(i in seq(from=1,to=nrow(grid))){
    ...
    if(mode=="transfer"){
      ...
      method <- c("separate","transfer","comb")
    } else if(mode=="multiple"){
      #--- multi-task learning ---
      data <- sim.data.multiple(prob.common=grid$prob.common[i],prob.separate=grid$prob.separate[i],family=grid$family[i])
      method <- c("separate","mgaussian","comb") # add glmnet "mgaussian" (only for linear case) or "MTPS" (not sparse)?
    }
...
```{r simulation_figure,eval=FALSE}
#<<setup>>
...
grDevices::postscript(file=file.path(path,"manuscript","fig_sim_multiple.eps"))
...
grDevices::postscript(file=file.path(path,"manuscript","fig_sim_transfer.eps"))
...
# select most expressed protein-coding genes (for all projects together)
select <- list()
total <- numeric()
for(i in unlist(project)){
...
temp <- cond & apply(total,2,function(x) all(x>0)) & (mean_rank >= sort(mean_rank[cond],decreasing=TRUE)[2000])
for(i in unlist(project)){
  select[[i]] <- temp
}
...
```{r explore_tables,eval=TRUE,results="asis"}
#<<setup>>
...
for(i in names(project)){
  ...
  for(j in seq_len(5)){ # 5 repetitions of 10-fold CV
    set.seed(j)
    codes <- project[[i]]
    result[[i]][[j]] <- crossval(y=y[codes],X=x[codes],family="binomial",method=c("separate","transfer","comb","common"),nfolds=10,alpha=1,alpha.init=alpha.init,type=type)
  }
}
...
```{r transfer_figure,eval=TRUE}
#<<setup>>

grDevices::postscript(file=file.path(path,"manuscript","fig_app.eps"),width=6,height=4)
graphics::par(mfrow=c(2,1),mar=c(4,4,1,1),oma=c(0,0,0,0))
...
xlab <- label[index]
change(x=xlab,y0=metric[,reference],y1=metric[,experimental],main="metric",increase=TRUE)
...
change(x=xlab,y0=nzero[,reference],y1=nzero[,experimental],main="sparsity",increase=FALSE)
graphics::axis(side=1,at=length(project$IBD)+0.5,labels="|",tick=FALSE,line=-0.25,font=2)
graphics::abline(h=0,lty=2,col="grey")
grDevices::dev.off()

# percentage change (keep this)
...
```{r interpret_models,eval=TRUE,fig.height=4,fig.width=7}
#<<setup>>
...
for(i in seq_along(result$IBD)){
  coefs[[i]] <- result$IBD[[i]]$refit$coef$comb
  colnames(coefs[[i]]) <- names(project$IBD)
  rownames(coefs[[i]]) <- rownames(result$IBD[[1]]$refit$coef$transfer) # try to avoid this
}
