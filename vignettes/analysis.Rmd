...
for(mode in c("TL-source","TL-target","MTL-size","TL-prop","MTL-prop")){
  ...
  for(i in seq(from=1,to=nrow(grid[[mode]]))){
      ...
      if(mode %in% c("TL-source","TL-target","TL-prop")){
        ...
        method <- c("glm.separate","glm.transfer","sparselink","glm.xrnet")
...
for(mode in c("TL-source","TL-target","MTL-size","TL-prop","MTL-prop")){
  ...
  if(mode %in% c("TL-source","TL-target","TL-prop")){
    col <- c("glm.separate"="black","glm.transfer"="red","glm.xrnet"="orange","sparselink"="blue")
    lty <- c("glm.separate"=3,"glm.transfer"=2,"glm.xrnet"=2,"sparselink"=1)
  } else if(mode %in% c("MTL-size","MTL-prop")) {
