...
for(i in names(project)){
  ...
  for(j in seq_len(5)){ # 5 repetitions of 10-fold CV
    set.seed(j)
    codes <- project[[i]]
    result[[i]][[j]] <- cv_transfer(y=y[codes],X=x[codes],family="binomial",method=c("glm.separate","glm.glmtrans","sparselink","glm.xrnet"))
  }
}
...
for(i in seq_along(prob_mtl)){
  ...
  for(j in seq_len(iter)){
    ...
    #auc_mtl[[i]][[j]] <- cv_multiple(y=Y,X=x,method=c("glm.separate","sparselink"),family="binomial",alpha.init=0.95,alpha=1,type="exp",trial=FALSE)$auc
...
for(i in seq_along(prob_tl)){
  ...
  for(j in seq_len(iter)){
    ...
    auc_tl[[i]][[j]] <- cv_transfer(y=Y,X=X,method=c("glm.separate","sparselink"),family="binomial",alpha.init=0.95,alpha=1,type="exp",trial=FALSE)$auc
  }
}
