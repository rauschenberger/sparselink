...
temp <- cond & mean >= sort(mean[cond],decreasing=TRUE)[2000] # change number
select[[extra]] <- temp
...
# DAS score defines disease activity!
#
#X <- x$SRP129004
...
for(k in seq_along(problem)){
  ...
  for(i in seq_len(1)){ # 5 repetitions of 10-fold CV
    set.seed(i)
    method <- c("glm.empty","glm.separate","glm.mgaussian","sparselink","group.devel")
...
lapply(test,function(x) sapply(x,function(x) colMeans(t(x)/x["none",])))

sapply(test,function(x)
rowMeans(sapply(x,function(x) rowMeans(apply(x,2,rank)))))
...
for(i in seq_len(nrow(setting))){
  ...
  #metric[[i]] <- joinet:::cv.joinet(Y=y[cond,vars],X=x[cond,],family="gaussian")
...
frac <- sapply(X=metric,FUN=function(x) rowMeans(x/x["none",]))
