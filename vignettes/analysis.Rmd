...
for(i in seq_along(vars)){
  ...
  if(sum(cond)>100){cond[cumsum(cond)>100] <- FALSE} # temporary
  #metric[[i]] <- joinet:::cv.joinet(Y=y[cond,cols],X=x[cond,],family="gaussian",alpha.base=1,alpha.meta=1)
  metric[[i]] <- cv.multiple(y=y[cond,cols],X=x[cond,],family="gaussian",method="sparselink",nfolds=10,alpha.init=0.95,alpha=1,type="exp",trial=TRUE)
}
