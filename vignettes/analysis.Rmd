...
cond <- apply(X=Y,MARGIN=1,FUN=function(x) all(!is.na(x))) # & pathotype=="lymphoid"
y <- as.matrix(Y)[cond,]
x <- X[cond,]
...
#--- examine correlation between coefficients ---
...
for(k in seq_along(problem)){
  ...
  for(i in seq_len(1)){ # 5 repetitions of 10-fold CV
    ...
    method <- c("glm.empty","glm.separate","glm.mgaussian","sparselink","devel")
