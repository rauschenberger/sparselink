...
for(i in seq_along(prob_tl)){
  ...
  for(j in seq_len(iter)){
    ...
    auc_tl[[i]][[j]] <- crossval(y=Y,X=X,method=c("glm.separate","sparselink"),family="binomial",alpha.init=0.95,alpha=1,type="exp")$auc
  }
}
...
auc <- sapply(auc_mtl,function(x) colMeans(x[[1]]))
...
auc <- sapply(auc_tl,function(x) x[[1]][2,])
