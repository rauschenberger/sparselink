...
pkgs <- c("knitr","rmarkdown","glmnet","BiocManager","spls","xrnet")
...
for(i in seq_along(prob_mtl)){
  ...
  for(j in seq_len(iter)){
    ...
    #auc_mtl[[i]][[j]] <- cv.multiple(y=Y,X=x,method=c("glm.separate","sparselink"),family="binomial",alpha.init=0.95,alpha=1,type="exp",trial=TRUE)$auc
...
for(i in seq_along(prob_tl)){
  ...
  for(j in seq_len(iter)){
    ...
    auc_tl[[i]][[j]] <- crossval(y=Y,X=X,method=c("glm.separate","sparselink"),family="binomial",alpha.init=0.95,alpha=1,type="exp",trial=TRUE)$auc
  }
}
