...
for(k in seq_along(problem)){
  results[[k]] <- list()
  for(i in seq_len(2)){ # 5 repetitions of 10-fold CV
    set.seed(i+1)
    method <- c("glm.empty","glm.separate","glm.mgaussian","devel") #"sparselink" "group.devel"
...
Y <- read.csv(paste0(path,"/results/PPMI_Curated_Data_Cut_Public_20250321.csv"))
colnames(Y) <- tolower(colnames(Y))

Y <- Y[Y$cohort==1 & Y$event_id %in% c("V04","V06","V08"),]
